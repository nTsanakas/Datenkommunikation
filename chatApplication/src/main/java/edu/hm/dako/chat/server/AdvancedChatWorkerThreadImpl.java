...
public class AdvancedChatWorkerThreadImpl extends SimpleChatWorkerThreadImpl {
	...
	protected static ConcurrentMap<String, ConfirmThread> confirmThreads = new ConcurrentHashMap<String, ConfirmThread>();
	...
	protected void eventResponseAction(ChatPDU receivedPdu) {
		...
		ConfirmThread ct = null;
		try {
			ct = confirmThreads.get(key);
			if (ct.removeClientAsConfirmed(userName)) {
				confirmThreads.remove(key);
			}		
		} catch (NullPointerException np) {
			log.debug("Confirm Thread war nicht zu finden: " + key);
			System.err.println("Confirm Thread war nicht zu finden: " + key + " User war: " + userName);
//			np.printStackTrace();
		}
	}
	...
	protected class ConfirmThread extends Thread {
		...
		public void run() {
			...
			responsePdu.setNumberOfLostEventConfirms(numberOfLostConfirms.get());
			responsePdu.setNumberOfReceivedEventConfirms(numberOfReceivedConfirms.get());
			try {
		 ...
		 * @return  True if it was the last client, else false.
		 */
		protected boolean removeClientAsConfirmed(String name) {
			clientsNotYetConfirmed.remove(name);
			numberOfReceivedConfirms.getAndIncrement();
			return clientsNotYetConfirmed.isEmpty();
		}
